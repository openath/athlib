...
def _scoring_objects_create():
    """Function to handle lazy evaluation of _scoring_objects, which maps from scoring key to parameters."""
...
def score(gender, event_code, value):
    ...
    def fnew(gender, event_code, value):
        ...
        if _jump_codes.match(event_code):
            if value > coeffs["Z"]:
                return max(0, int(coeffs["A"] * ((value - coeffs["Z"]) ** coeffs["X"])))
            else:
                return 0
            ...
            if coeffs["Z"] > value:
                return max(0, int(coeffs["A"] * ((coeffs["Z"] - value) ** coeffs["X"])))
            else:
                return 0
...
def performance(gender, event_code, score):
    ...
    def fnew(gender, event_code, score):
        ...
        if _jump_codes.match(event_code):
            return int(math.ceil(((score / coeffs["A"]) ** (1.0 / coeffs["X"])) + coeffs["Z"]))
        elif _throw_codes.match(event_code):
            return math.ceil(100.0 * (((score / coeffs["A"]) ** (1.0 / coeffs["X"])) + coeffs["Z"])) / 100.0
        else:
            return math.floor(100.0 * (coeffs["Z"] - ((score / coeffs["A"]) ** (1.0 / coeffs["X"])))) / 100.0
